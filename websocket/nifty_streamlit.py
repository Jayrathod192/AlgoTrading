...
OI_POLL_INTERVAL = 30  # seconds
# ----------------------------------------

# Streamlit setup
st.set_page_config(page_title="Nifty Live OI Dashboard", layout="wide")
st.title("📊 Nifty 50 Live Candlestick + OI Mirror Chart + ΔOI Bar Chart")

placeholder = st.empty()

# --- Add this global storage at top of file ---
        ...
        with self.lock:
            ...
            if candles:
                ...
                for i, c in enumerate(candles):
                    ...
                    rect = Rectangle((i - 0.3, min(c['open'], c['close'])), 0.6,
                                     abs(c['close'] - c['open']), facecolor=color, edgecolor='black')
                    ax1.add_patch(rect)
                ax1.set_title("NIFTY Candlestick + CE/PE OI Overlay")
                ax1.set_ylabel("Price")

            # ---- Horizontal OI Mirror Chart ----
            if self.oi_data:
                df_oi = pd.DataFrame(self.oi_data).T
                if not {'CE', 'PE'}.issubset(df_oi.columns):
                    return fig  # Skip plotting until valid data arrives
                ...
                for strike in df_oi.index:
                    ...
                    ax1.text(-ce - 0.3, strike, f"{df_oi['CE'][strike]/1e5:.1f}L",
                             color='red', va='center', ha='right', fontsize=8)
                    # PE on right
                    ax1.barh(strike, pe, color='green', alpha=0.5)
                    ax1.text(pe + 0.3, strike, f"{df_oi['PE'][strike]/1e5:.1f}L",
                             color='green', va='center', ha='left', fontsize=8)

                ax1.set_xlabel("OI Strength (Scaled)")
                ax1.grid(True, alpha=0.3)

            # ---- ΔOI Chart ----
            if self.prev_oi_data is not None and self.oi_data:
                df_oi = pd.DataFrame(self.oi_data).T
                df_oi.index = df_oi.index.astype(int)
                df_oi = df_oi.sort_index()
                ...
                df_prev = self.prev_oi_data.reindex(df_oi.index).fillna(0)
                if not {'CE', 'PE'}.issubset(df_prev.columns):
                    df_prev = df_oi.copy()

                df_change = df_oi[['CE', 'PE']] - df_prev[['CE', 'PE']]

                ax2.bar(df_change.index - 50, df_change['CE'] / 1e6, width=80,
                        color='red', alpha=0.6, label='CE ΔOI')
                ax2.bar(df_change.index + 50, df_change['PE'] / 1e6, width=80,
                        color='green', alpha=0.6, label='PE ΔOI')
                ax2.set_title("OI Change (ΔOI per Strike in Millions)")
                ax2.legend()
                ax2.grid(True, alpha=0.3)

            self.prev_oi_data = pd.DataFrame(self.oi_data).T.sort_index()
            plt.tight_layout()
            return fig
